	...
	public void handle(CommandContext ctx) {
		...
		if (ctx.getChannel().getId().equals("793992497264984074")) {
			if (ctx.getArgs().size() == 0) {
				...
				eb.setImage(ctx.getMember().getUser().retrieveProfile().complete().getBannerUrl() + "?size=1024");
				...
				eb.setTimestamp(Instant.now());
				eb.setColor(Color.GREEN);
				eb.setImage(member.getUser().retrieveProfile().complete().getBannerUrl() + "?size=1024");
				ctx.getChannel().sendMessageEmbeds(eb.build())
						.queue(msg -> msg.delete().queueAfter(1, TimeUnit.MINUTES));
