...
public class Paginator extends Menu {
	...
	public static final String STOP = "U+1F6D1";
	...
	private void initialize(RestAction<Message> action, int pageNum) {
		action.queue(m -> {
			if (pages > 1) {
				...
				m.addReaction(Emoji.fromUnicode(STOP)).queue(v -> paginationWithoutTextInput(m, pageNum),
	...
	private boolean checkReaction(MessageReactionAddEvent event, long messageId) {
		if (event.getMessageIdLong() != messageId)
			return false;
		switch (event.getReaction().getEmoji().asUnicode().getFormatted()) {
	...
	private Message renderPage(int pageNum) {
		...
		if (showPageNumbers)
			ebuilder.setFooter("Página " + pageNum + "/" + pages, null);
		User u = ((User) users.toArray()[0]);
		if (u != null) {
			ebuilder.setAuthor(u.getName(), u.getAvatarUrl(), u.getAvatarUrl());
		}
